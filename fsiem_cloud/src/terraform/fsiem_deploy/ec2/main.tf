...
data "aws_secretsmanager_secret" "fortimonitor" {
  arn = local.fortimonitor_arn[var.environment]
}

data "aws_secretsmanager_secret_version" "fortimonitor" {
  secret_id = data.aws_secretsmanager_secret.fortimonitor.id
}
